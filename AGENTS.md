...
> **Graft note (2026-09-10):** this repo was scaffolded by `hackathon-setup` before the bus was
> grafted on. Its original `AGENTS.md` — the hackathon's surface-ownership / claims-table
> concurrency model — now lives at `AGENTS-project.md`. **This file (the bus protocol) is
> authoritative for claiming, lanes, and PRs**, per that file's own "Forward-looking" section,
> which anticipated exactly this graft: the `claim/<n>` git ref is the lock, superseding
> `STATE.md`'s claims table, with truth ordering refs > CI > labels. `AGENTS-project.md` still
> applies for what it uniquely covers and this file doesn't: within-lane surface-ownership
> discipline, the escalation rule for conflicting work, and the end-of-session checklist. Read
> both; where they conflict on *claiming*, this file wins.
